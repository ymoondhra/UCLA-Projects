    ...
    while(1) {
        ...
        if(now.tv_sec >= next.tv_sec && running) {
            ...
            if(SSL_write(ssl, data, strlen(data)) <= 0) {
				fprintf(stderr, "Error: could not write data through SSL");
				exit(2);
			}
			if(logOption) {
        ...
        if(poller < 0) {
            ...
            if(polls[0].revents & POLLIN) {
                int numRead = SSL_read(ssl, buffer, 1000);
                if(numRead <= 0) {
                    fprintf(stderr, "Error: could not read through SSL\n");
                    exit(2);
                }
                int x = 0;
                int copyx = 0;
...
void clientServer() {
  ...
  if(connect(socket_fd, (struct sockaddr *) &server, sizeof(server)) < 0) {
   ...
   ssl_context = SSL_CTX_new(SSLv23_client_method());
	if (ssl_context == NULL)
	{
		fprintf(stderr, "Error: culd not create SSL context\n");
		exit(2);
	}
	...
	if (SSL_set_fd(ssl, socket_fd) == 0) {
		fprintf(stderr,"Error: could not link socket and SSL\n");
		exit(2);
	}
	if (SSL_connect(ssl) <= 0) {
		SSL_shutdown(ssl);
		SSL_free(ssl);
		fprintf(stderr,"Error: could not connect with SSL\n");
		exit(2);
	}
  ...
  int written = SSL_write(ssl, stringID, strlen(stringID));
  if(written <= 0) {
    fprintf(stderr, "Error: could not write ID through SSL\n");
    exit(2);
  }
