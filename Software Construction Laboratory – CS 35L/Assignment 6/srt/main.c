...
/* VARIBLES */
int nthreads;
scene_t scene; //all threads must have access to the same
//value of scene
typedef struct {
    float first;
    float second;
...
static void* picSplit (void* threadNum) {
    int currThread = *((int *) threadNum);

    int start = (width/nthreads)*currThread;
    int end = (width/nthreads)*(currThread + 1);
    if(currThread + 1 == nthreads) {
        end = width;
    }
    ...
    for( int px=start; px<end; ++px )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            values[px*height + py].first = gamma( pixel_color[0] ) * max_color;
            values[px*height + py].second = gamma( pixel_color[1] ) * max_color;
            values[px*height + py].third = gamma( pixel_color[2] ) * max_color;

            /* enforce caps, replace with real gamma */
            values[px*height + py].first = max( min(values[px*height + py].first, 255), 0);
            values[px*height + py].second = max( min(values[px*height + py].second, 255), 0);
            values[px*height + py].third = max( min(values[px*height + py].third, 255), 0);
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    values = malloc(width*height*sizeof(valuesStruct));
    ...
    for (i = 0; i < nthreads; i++) {
      if(pthread_create(&threads[i], NULL, picSplit, &threadID[i])) {
        fprintf(stderr, "Failed to generate new thread");
        exit(1);
      }
    }
    ...
    for (i = 0; i < width; i++)
    {
      ...
      {
	      printf( "%.0f %.0f %.0f\n",
		      values[i*height + j].first, values[i*height + j].second, values[i*height + j].third);
      }
      printf("\n");
    }
