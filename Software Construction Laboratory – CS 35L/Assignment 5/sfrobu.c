#include <string.h>
#include <stdlib.h>
#include <stdbool.h>
#include <unistd.h>
#include <sys/stat.h>
#include <stdio.h>
#include <ctype.h>
//takes the XOR of int 42 with char c. This works    
//because char c can be converted to int
char defrob(const char c) {
    return c^42;
}
char defrobUpper(const char c) {
  return ((toupper(c))^42);
}
//compares two strings to see if one is greater or they
//are both equal                                                
int frobcmp(char const * a, char const * b) {
    for(;; a++, b++) {
        while(*a == '\0')
        {
            a++;
        }
        while(*b == '\0')
        {
            b++;
        }
        if(*a == ' ' && *b == ' ') {
            return 0; }
        else if((*a == ' ') || (defrob(*a) < defrob(*b)) ) {
            return -1;
        }
        else if((*b == ' ')|| (defrob(*a) > defrob(*b)) ) {
            return 1;
        }
    }
}
int frobcmpUpper(char const * a, char  const * b) {
  for(;; a++, b++) {
    while(*a == '\0') {
      a++;
    }
    while(*b == '\0') {
      b++;
    }
    if(*a == ' ' && *b == ' ') {
      return 0;
    }
    else if((*a == ' ') || (defrobUpper(*a) < defrobUpper(*b)) ) {
      return -1;
    }
    else if((*b == ' ') || (defrobUpper(*a) > defrobUpper(*b))) {
      return 1;
    }
  }
}
//comparing two c-strings using frobcmp     
int compare(const void* x, const void* y) {
    char const* first = *(const char **) x;
    char const* second = *(const char **) y;
    return frobcmp(first, second);
}
int compareIgnoreCase(const void* x, const void* y) {
  char const* first = *(const char**) x;
  char const* second = *(const char **) y;
  return frobcmpUpper(first,second);
}
//checks for io error                                                                     
int IOErrorCheck(void) {
    if(ferror(stdin)) {
        fprintf(stderr, "There is an Input / Output Error. Exiting now. \n");
        return 0;
    }
    return 1;
}

int main(int argc, char **argv) {
  bool flag = false;
  if(argc == 2) {
    if(strcmp(argv[1], "-f") == 0) {
      flag = true;
    }
    else {
      fprintf(stderr, "Error: option is not f");
      exit(1);
    }
  }

  struct stat finfo;
  
  if(fstat(0, &finfo) < 0) {
    fprintf(stderr, "Error in reading file");
    exit(1);
  } //returns set of attributes to the structure
  int numBytes = 0;
  if(S_ISREG(finfo.st_mode)) {
    numBytes = finfo.st_size;
  }

  char* charfile = malloc(numBytes);
  if(charfile == NULL) {
    fprintf(stderr, "Memory Allocation Failed\n");
    exit(1);
  }
  read(0, charfile, numBytes);

  char* cs;
  char** cstringarray;

  int x = 0;
  int y = 0;
  int i = 0; //ADDED THIS!~!!!!

  cs = (char*)malloc(sizeof(char));
  cstringarray = malloc(sizeof(char*));
  
  bool newword = true;

  //FOR FILE INPUT
  for(i = 0; i < numBytes; i++) {
    if(!IOErrorCheck()) {
      free(cs);
      for(int j = 0; j < y; j++) {
	       free(cstringarray[j]);
      }
      free(cstringarray);
      free(charfile);
      exit(1);
    }

    char next = charfile[i];
    //hackee
     if((next == '\n') && (i == numBytes - 1)) {
      next = ' ';
    }
    char* tr = realloc(cs, x*sizeof(char) + sizeof(char));
    if(tr != NULL) {
      cs = tr;
      cs[x] = next;
    }
    else {
      free(cs);
      for(int q = 0; q < y; q++) {
        free(cstringarray[q]);
      }
      free(cstringarray);
      free(charfile);
      fprintf(stderr, "Memory Allocation Failed\n");
      exit(1);
    }

    if(next == ' ') {
      if(newword) {
      	newword = false;
      	char** t = realloc(cstringarray, y*sizeof(char*) + sizeof(char*));

      	if(t != NULL) {
      	  cstringarray = t;
      	  cstringarray[y] = cs;
      	  y++;
      	  x = -1;
      	  cs = (char*)malloc(sizeof(char));
      	}
      	else {
      	  free(cs);
      	  for(int i = 0; i < y; i++) {
      	    free(cstringarray[i]);
      	  }
      	  free(cstringarray);
      	  free(charfile);
      	  fprintf(stderr, "Memory Allocation Failed\n");
      	  exit(1);
      	}
      }
      else {
	     newword = false;
	     x = -1;
      }
    }
    else {
      newword = true;
    }
    x++;
  }

  
  x = 0;
  int stat = read(0, &cs[0], sizeof(char)); //random value
  newword = true;
  
  //FOR STANDARD INPUT  
  while((stat > 0) && (IOErrorCheck())) {
    if(!IOErrorCheck()) {
        free(cs);
        for (int i = 0; i < y; i++)
            free(cstringarray[i]);
        free(cstringarray);
        exit(1);
      }
    
    if(cs[x] == ' ') {
      if(newword) {
        newword = false;
  
	char** t = realloc(cstringarray, y*sizeof(char*) + sizeof(char*));
  
        if(t != NULL) {
	  cstringarray = t;
	  cstringarray[y] = cs;
	  y++;
	  x = -1;
	  cs = (char*)malloc(sizeof(char));
        }
        else {
          free (cs);
          for (int i = 0; i < y; i++)
              free(cstringarray[i]);
          free(cstringarray);
          fprintf(stderr, "Memory Allocation Failed\n");
          exit(1);
	}
      }
      else {
	newword = false;
	x = -1;
      }
    }
    else {
      newword = true;
    }

    x++;
    char* tr = realloc(cs, x*sizeof(char) + sizeof(char));
    if(tr != NULL) {
      cs = tr;
      stat = read(0, &cs[x], sizeof(char));
    }
    else {
      free(cs);
      for(int q = 0; q < y; q++) {
        free(cstringarray[q]);
      }
      free(cstringarray);
      fprintf(stderr, "Memory Allocation Failed\n");
      exit(1);
    }
  }
  if(x > 0 && newword) {
      char** t = realloc(cstringarray, y*sizeof(char*) + sizeof(char*));
      if(t != NULL) {
        cstringarray = t;
        cstringarray[y] = cs;
        y++;
        x = -1;
        cs = (char*)malloc(sizeof(char));
      }
      else {
        free(cs);
        for(int i = 0; i < y; i++) {
          free(cstringarray[i]);
        }
        free(cstringarray);
        free(charfile);
        fprintf(stderr, "Memory Allocation Failed\n");
        exit(1);
      }
  }

  if(flag) {  
    qsort(cstringarray, y, sizeof(char*), compareIgnoreCase);
  }
  else {
    qsort(cstringarray, y, sizeof(char*), compare);
  }

  //send to stdout
  int a;
  int b;
  for(a = 0; a < y; a++) {
    printf("%s", cstringarray[a]);
  }

  //free up the memory                                                        
  for(a = 0; a < y; a++) {
      free(cstringarray[a]);
  }
  free(cstringarray);
  free(cs);
  free(charfile);
  return 0;
}
